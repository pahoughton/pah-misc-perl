...
# implied warranty.
#
# This code parses the .emlx files used by Mail.app in MacOS 10.4 - 10.7.
# Mail.app stores folders as directories with one file per message.
# Prior to 10.7, all the messages were in a single folder; in 10.7 and
# later, they are distributed into subdirectories.
#
...
sub showfile($$$$) {
  ...
  if ($list_p) {
    ...
    # These are not always present in props, as of 10.7.
    ($date) = ($body =~ m/^Date\s*:\s*(.+?)\n/mi)    unless $date;
    ($from) = ($body =~ m/^From\s*:\s*(.+?)\n/mi)    unless $from;
    ($subj) = ($body =~ m/^Subject\s*:\s*(.*?)\n/mi) unless $subj;

    $date = str2time ($date) unless ($date =~ m/^\d+$/s);
    ...
    if ($mbox_p == 1) {
      ...
      $body =~ s/^(-{10})/- $1/gm;  # quoting for digests
      my $hr = '-' x 72;
      print "$headers2\n$body\n\n$hr\n";
    }
  }
...
sub usage() {
  print STDERR "usage: $progname [--verbose] " .
    "[--unread|--all] [--list|--show|--mbox|--ids] file.emlx ...\n";
  exit 1;
}
...
sub main() {
  ...
  # Sort files by write date, now that they are scattered in subdirs.
  my %date;
  foreach (@files) { $date{$_} = (stat($_))[9] || 9999999999; }
  @files = sort { $date{$a} <=> $date{$b} } @files;
